...
public class Model {
        ...
        static def initStringQueries(){
                logger.info("adding StringQueries overload")
                addStringExecuteQuery()
                addStringExecuteUpdate()
                addGStringExecuteUpdate()
                addStringExecuteScalar()
                logger.info("StringQueries added")
        }

        public static Object list(String cls, LinkedHashMap optionsMap) {
...
//                                           if (isNumeric(limit))
        ...
        public static Object findAll(String cls, Map whereMap) {
                Class clazz = Class.forName(cls, false, ThreadBag.get().groovyClassLoader)
...
//                                sql << "$columnName = ?  $operator "
                        //                                values << getValue(it?.url?.class,it?.url)
                }
                ...
                db.eachRow(new GStringImpl(values.toArray(), sql.toString().trim().split('\\?'))) { rs ->
                        ...
                        if (row.metaClass.respondsTo(row, "clearDynamicProperties")) {
        ...
        def update() {
                ...
                this.dynamicProperties.each {
                        ...
                        def val = this."$it"
                        values << Model.getValue(val?.class, val)

                        //values << getValue(_type?.class, this?."$it")
                }
        ...
        def insert(Boolean insertUpdatedColumnsOnly) {
                ...
                db.executeInsert(new GStringImpl(values.toArray(), s.toString().trim().split('\\?')))
        ...
        private static def getTableName(Class clazz) {
                ServletContext app = ThreadBag.get().request.servletContext
        ...
        private static def createSelectList(clazz, final boolean hasColumnsProperty, sql, Map columns, Object thisObject) {
                clazz.declaredFields.each {
                        def prop = clazz.metaClass.getMetaProperty(it.name)
                        def exclude = false;

                        if (Model.isProperty(clazz, 'exclude')) {
                                ...
                                if (clazz.metaClass.getMetaProperty(it.name) && !it.name.equals('metaClass') && (prop instanceof MetaBeanProperty)) {
                                        if (!prop.field.isStatic()) {
                                                if (hasColumnsProperty && thisObject.columns?.containsKey(it.name)) {
                                                        ...
                                                        sql << Model.unCamelCaseColumn(it.name) << ' as ' << it.name
                                                        columns[it.name] = Model.unCamelCaseColumn(it.name)
                                                }
                                                sql << ', '
        ...
        def static Sql getSqlInstance(String dataSourceName) {
                ...
                def db = ThreadBag.get()?.sql
                if (db != null) return db


                logger.finest 'newSqlInstanceMethodAvailable is true'
                Db.newSqlInstance(dataSourceName)
...
//                        return newSqlInstanceMethod.invoke(null, null)
        ...
        static def createPagingQuery(def sql, def page, int pageSize) {
                ServletContext app = ThreadBag.get().request.servletPath
...
//                        String[] values = new String[0]
        ...
        public static void addStringExecuteQuery() {
                ...
                GString.metaClass.executeQuery = { Object[] args ->
                        ...
                        if (clazz == null) {
                                ...
                                Map columns = [:]
                                boolean hasColumnsProperty = Model.isProperty(clazz, 'columns')
        ...
        public static boolean doesColumnExists() {
